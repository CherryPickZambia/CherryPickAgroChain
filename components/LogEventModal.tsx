                                ...
                                <div className="grid grid-cols-5 gap-3">
                                    {imagePreviews.map((src, idx) => (
                                        <div
                                            ...
                                            className={`relative aspect-square group rounded-2xl overflow-hidden border-2 cursor-pointer transition-all ${selectedAiImageIndex === idx ? "border-emerald-500 ring-2 ring-emerald-100" : "border-blue-100 hover:border-emerald-300"}`}
                                        >
                                            <img src={src} alt="Preview" className="w-full h-full object-cover" />
                                            <div className="absolute left-1 bottom-1 px-1.5 py-0.5 rounded-full bg-black/60 text-white text-[8px] font-semibold">
                                                {selectedAiImageIndex === idx ? "AI" : idx + 1}
                                            </div>
                                            <button
                                                type="button"
                                ...
                                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 space-y-3">
                                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                        <div>
                                            ...
                                            <p className="text-[10px] text-emerald-700 mt-1">Detect disease and assess crop health on your evidence photo.</p>
                                        </div>
                                        <button
                                            type="button"
                                            onClick={handleAnalyzeEvidencePhoto}
                                            disabled={aiAnalyzing || selectedAiImageIndex === null || !evidenceImages[selectedAiImageIndex ?? -1]}
                                            className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-300 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-colors flex items-center justify-center gap-2"
                                        >
                                            {aiAnalyzing ? (
                                                <><Loader2 className="w-4 h-4 animate-spin" /> Analyzing...</>
                                            ) : (
                                                <><CheckCircle className="w-4 h-4" /> {previousDiagnosis ? "Verify Recovery" : "Analyze Photo"}</>
                                            )}
                                        </button>
                                    </div>
                                    ...
                                    {previousDiagnosis && !aiResult && (
                                        <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl">
                                            ...
                                            <p className="text-[11px] text-amber-700">
                                                ...
                                                Analyze a fresh evidence photo to verify recovery.
                                            </p>
                                        </div>
                                    )}
                                    ...
                                    {evidenceImages.length > 0 && (
                                        <textarea
                                            ...
                                            placeholder="Optional context for the AI, e.g. spots on lower leaves, recent pest activity..."
                                            rows={2}
